...
static struct accton_as7312_54x_fan  *fan_data = NULL;

struct accton_as7312_54x_fan {
...
/*******************/
static void accton_as7312_54x_fan_update_device(struct device *dev);
static int accton_as7312_54x_fan_read_value(u8 reg);
static int accton_as7312_54x_fan_write_value(u8 reg, u8 value);
...
static struct attribute *accton_as7312_54x_fan_attributes[] = {
             ...
             char *buf)
{
    ...
    accton_as7312_54x_fan_update_device(dev);
...
static ssize_t fan_set_duty_cycle(struct device *dev, struct device_attribute *da,
    ...
    accton_as7312_54x_fan_write_value(CPLD_REG_FAN_PWM_CYCLE_OFFSET, value/FAN_SPEED_PRECENT_TO_CPLD_STEP);
...
static const struct attribute_group accton_as7312_54x_fan_group = {
    .attrs = accton_as7312_54x_fan_attributes,
};

static int accton_as7312_54x_fan_read_value(u8 reg)
{
    return as7312_54x_i2c_cpld_read(0x60, reg);
}

static int accton_as7312_54x_fan_write_value(u8 reg, u8 value)
{
    return as7312_54x_i2c_cpld_write(0x60, reg, value);
}
...
static void accton_as7312_54x_fan_update_device(struct device *dev)
{
    ...
    if (LOCAL_DEBUG)
        printk ("Starting accton_as7312_54x_fan update \n");
    ...
    if (LOCAL_DEBUG)
        printk ("Starting accton_as7312_54x_fan update 2 \n");

    fault      = accton_as7312_54x_fan_read_value(CPLD_REG_FAN_STATUS_OFFSET);
    r_fault    = accton_as7312_54x_fan_read_value(CPLD_REG_FANR_STATUS_OFFSET);
    direction  = accton_as7312_54x_fan_read_value(CPLD_REG_FAN_DIRECTION_OFFSET);
    ctrl_speed = accton_as7312_54x_fan_read_value(CPLD_REG_FAN_PWM_CYCLE_OFFSET);
    ...
    for (i=0; i<FAN_MAX_NUMBER; i++)
    {
        ...
        /* fan speed
         */
        speed      = accton_as7312_54x_fan_read_value(fan_speed_reg[i]);
        r_speed    = accton_as7312_54x_fan_read_value(fanr_speed_reg[i]);
...
static int accton_as7312_54x_fan_probe(struct platform_device *pdev)
{
    int status = -1;

    /* Register sysfs hooks */
    status = sysfs_create_group(&pdev->dev.kobj, &accton_as7312_54x_fan_group);
    if (status) {
        goto exit;
    ...
    dev_info(&pdev->dev, "accton_as7312_54x_fan\n");

    return 0;

exit_remove:
    sysfs_remove_group(&pdev->dev.kobj, &accton_as7312_54x_fan_group);
exit:
    return status;
}

static int accton_as7312_54x_fan_remove(struct platform_device *pdev)
{
    hwmon_device_unregister(fan_data->hwmon_dev);
    sysfs_remove_group(&fan_data->pdev->dev.kobj, &accton_as7312_54x_fan_group);
...
static struct platform_driver accton_as7312_54x_fan_driver = {
    .probe      = accton_as7312_54x_fan_probe,
    .remove     = accton_as7312_54x_fan_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
static int __init accton_as7312_54x_fan_init(void)
{
    int ret;
    ...
    ret = platform_driver_register(&accton_as7312_54x_fan_driver);
    if (ret < 0) {
        goto exit;
    }

    fan_data = kzalloc(sizeof(struct accton_as7312_54x_fan), GFP_KERNEL);
    if (!fan_data) {
        ret = -ENOMEM;
        platform_driver_unregister(&accton_as7312_54x_fan_driver);
        goto exit;
    }
...
static void __exit accton_as7312_54x_fan_exit(void)
{
...
MODULE_DESCRIPTION("accton_as7312_54x_fan driver");
MODULE_LICENSE("GPL");

module_init(accton_as7312_54x_fan_init);
module_exit(accton_as7312_54x_fan_exit);
